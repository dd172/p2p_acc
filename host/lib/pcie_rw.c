...
static int read_write_regs(int fd,void * fd_regs_map_base,char *devname, uint64_t addr, uint32_t writeval, int type)
{
...
void write_bram_reg(uint64_t offset,size_t size,void * src){
    ...
    read_write_regs(fd_regs,fd_regs_map_base,DEVICE_USR_DEFAULT, offset, writeval, 1);
}

void read_bram_reg(uint64_t offset,size_t size,void * dest){
    uint32_t writeval;
    writeval = read_write_regs(fd_regs,fd_regs_map_base,DEVICE_USR_DEFAULT, offset, 0, 0);
